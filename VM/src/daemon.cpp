...
void Daemon::Run(uint port)
{
    ...
    translator_ = std::make_unique<TranslateClient>(TranslateClient(grpc::CreateChannel(
      "translator:51899", grpc::InsecureChannelCredentials())));
    TranslateClient t2 = TranslateClient(grpc::CreateChannel(
      "translator:51899", grpc::InsecureChannelCredentials()));
...
Daemon::CallData::CallData(Simulator::AsyncService *service, ServerCompletionQueue *cq, TranslateClient *trans)
    : service_(service), cq_(cq), translator_(trans), responder_(&ctx_), status_(CREATE)
{
...
void Daemon::CallData::Proceed()
{
    if (status_ == CREATE)
    {
        ...
        service_->Requestsimulate(&ctx_, &request_, &responder_, cq_, cq_,
                                  this);
    }
    else if (status_ == PROCESS)
    {
        ...
        {
            reply_.set_status(semicode.status());
            reply_.set_result(semicode.code());
            status_ = FINISH;
...
void Daemon::HandleRpcs()
{
    ...
    new CallData(&service_, cq_.get(), translator_.get());
